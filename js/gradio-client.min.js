...
const MISSING_CREDENTIALS_MSG =
  'Login credentials are required to access this space.';
const NODEJS_FS_ERROR_MSG =
...
async function resolve_config(endpoint) {
  ...
  if (
    ...
    if (
      (response == null ? void 0 : response.status) === 401 &&
      !this.options.auth
    ) {
      ...
      Object.entries(api_info[category]).forEach(
        ([endpoint, {parameters, returns}]) => {
          ...
          if (
            dependencyIndex !== -1 &&
            ((_d =
              (_c = config.dependencies.find(
                dep => dep.id == dependencyIndex
              )) == null
                ? void 0
                : _c.inputs) == null
              ? void 0
              : _d.length) !== parameters.length
          ) {
            const components = config.dependencies
              .find(dep => dep.id == dependencyIndex)
              .inputs.map(input => {
                var _a2;
                return (_a2 = config.components.find(c => c.id === input)) ==
                  null
                  ? void 0
                  : _a2.type;
              });
          ...
          transformed_info[category][endpoint] = {
            parameters: parameters.map(p =>
              transform_type(
                p,
                p == null ? void 0 : p.component,
                p == null ? void 0 : p.serializer,
                'parameter'
              )
            ),
...
async function view_api() {
  ...
  try {
    ...
    if (typeof window !== 'undefined' && window.gradio_api_info) {
      ...
      if (
        semiver((config == null ? void 0 : config.version) || '2.0.0', '3.30') <
        0
      ) {
...
async function upload_files(root_url, files, upload_id) {
  ...
  const headers = {};
  if (
    (_a = this == null ? void 0 : this.options) == null ? void 0 : _a.hf_token
  ) {
...
async function upload(file_data, root_url, upload_id, max_file_size) {
  ...
  return await Promise.all(
    await this.upload_files(root_url, files, upload_id).then(async response => {
      if (response.error) {
        ...
        const array_refs = await walk_and_store_blobs(
          ...
          root
            ? ((_a =
                endpoint_info == null
                  ? void 0
                  : endpoint_info.parameters[index]) == null
                ? void 0
                : _a.component) || void 0
            : type,
          new_path,
...
async function process_local_file_commands(client2, data) {
  var _a, _b;
  const root =
    ((_a = client2.config) == null ? void 0 : _a.root) ||
    ((_b = client2.config) == null ? void 0 : _b.root_url);
  if (!root) {
...
async function duplicate(app_reference, options) {
  ...
  const {http_protocol, host} = await process_endpoint(app_reference, hf_token);
  let cookies = null;
  if (auth) {
    const cookie_header = await get_cookie_header(
      http_protocol,
  ...
  constructor(options = {allowCR: false}) {
    super({
      ...
      flush: controller => {
        ...
        const currentLine =
          options.allowCR && __privateGet(this, _currentLine).endsWith('\r')
...
function submit(endpoint, data = {}, event_data, trigger_id, all_events) {
  ...
  try {
    ...
    const events_to_publish =
      ((_a = options == null ? void 0 : options.events) == null
        ? void 0
        : _a.reduce((acc, event) => {
            acc[event] = true;
            return acc;
      ...
      async _payload => {
        ...
        if (skip_queue(fn_index, config)) {
          ...
          if (
            ...
            hostname =
              (_a2 = window == null ? void 0 : window.location) == null
                ? void 0
                : _a2.hostname;
          }
          ...
          const zerogpu_auth_promise = is_zerogpu_iframe
            ? post_message('zerogpu-headers', origin)
            ...
            if (status === 503) {
              ...
              let callback = async function (_data) {
                try {
                  ...
                  if (
                    (status2 == null ? void 0 : status2.stage) === 'complete' ||
...
class Client {
  constructor(app_reference, options = {events: ['data']}) {
    ...
    this.deep_link =
      ((_a = options.query_params) == null ? void 0 : _a.deep_link) || null;
    if (!options.events) {
      options.events = ['data'];
    }
  ...
  async init() {
    ...
    this.api_map = map_names_to_ids(
      ((_a = this.config) == null ? void 0 : _a.dependencies) || []
    );
  }
  ...
  async handle_space_success(status) {
    ...
    if (status.status === 'running') {
      try {
        ...
        this.api_prefix =
          ((_a = this == null ? void 0 : this.config) == null
            ? void 0
            : _a.api_prefix) || '';
        if (!this.config) {
          throw new Error(CONFIG_ERROR_MSG);
